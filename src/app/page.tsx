          ...
          <div className="mt-12 flex flex-col md:flex-row gap-8 items-start md:items-center">
             ...
             <FadeIn delay={0.5}>
                <div className="relative group">
                   ...
                   <Link href="#projects" className="relative block border-2 border-white bg-black px-8 py-4 text-xl font-bold uppercase tracking-wider hover:bg-white hover:text-black transition-colors">
                      Lihat Karya
                   </Link>
                </div>
             </FadeIn>
                   ...
                   <div className="flex flex-col lg:flex-row gap-8 lg:gap-16 items-center border-b-2 border-black/20 pb-12 last:border-0">
                      <div className="w-full lg:w-3/5">
                        <div className="relative border-4 border-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] bg-white hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all duration-300">
                          <img src={project.imageUrl || 'https://via.placeholder.com/800x600'} alt={project.title} className="w-full h-auto object-cover aspect-video grayscale hover:grayscale-0 transition-all duration-500"/>
                        </div>
                      </div>
                      <div className="w-full lg:w-2/5 space-y-6">
                        <div className="inline-block px-4 py-1 bg-black text-lime-400 font-bold uppercase text-xs tracking-widest rounded-full">Highlight</div>
                        <h3 className="font-syne text-5xl font-bold uppercase leading-none break-words">{project.title}</h3>
                        <p className="font-mono text-lg font-medium leading-relaxed opacity-80">{project.description}</p>
                        {project.link && (
                          <a href={project.link} target="_blank" className="inline-flex items-center gap-2 border-b-2 border-black pb-1 font-bold text-xl hover:gap-4 transition-all">Visit Site <span className="text-2xl">→</span></a>
                        )}
                      </div>
                   </div>
                 </FadeIn>
